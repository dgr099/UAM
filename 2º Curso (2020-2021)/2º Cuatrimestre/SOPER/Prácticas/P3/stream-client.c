...
int main (int argc, char **argv){
    ...
    if(argc < 2){
        ...
        perror("Error al intentar abrir el archivo de escritura\n");
        mq_close(queue);
        exit ( EXIT_FAILURE );
    }
    ...
    while(1){
        if(mq_receive(queue, instruccion, sizeof(instruccion), NULL) == -1){
            perror("Error recibiendo el mensaje en cliente");
            fclose(f);
            shm_unlink (SHM_NAME);
            mq_close(queue);
        ...
        if(sem_timedwait(&(shm_struct->sem[1]), &ts)==-1){ //espera 2 segundos a que haya alún hueco ocupado
                fprintf(stdout, "Tiempo pasado, operación get desechada\n");
                continue;
        }
        sem_wait(&(shm_struct->sem[0])); /*espero a poder entrar en el mutex*/
        
        /* Guardamos caracter a caracter desde el buffer hacia el archivo de salida */
        /* de forma circular hasta el final del buffer */
        if((character = shm_struct->buff[(shm_struct->get_pos)])!= '\0'){
            shm_struct->get_pos = (shm_struct->get_pos+1)%MAX_BUFF;
            fprintf(f, "%c", character);
        }
        
        sem_post(&(shm_struct->sem[0]));
        sem_post(&(shm_struct->sem[2]));
    }
}
