...
void queue_free(Queue *q){
  ...
  for(i=q->front, z=q->rear; i!=z; i=(i+1)%MAXQUEUE){
...
Status queue_insert(Queue *q, const void* pElem){
  ...
  if(queue_size(q)>=MAXQUEUE-1){ /*debe dejar uno libre para diferenciar entre llena y vacia, por lo que si size=MAXQUEUE-1 significa que está llena*/
    return ERROR;
  }
