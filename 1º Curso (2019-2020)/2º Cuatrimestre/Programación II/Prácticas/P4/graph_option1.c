/**************************************************
Nombre del modulo: graph_option1.c
...
struct _Graph {
Node *nodes[MAX_NODES]; /*!<Static array with the graph nodes */
List *plconnect[MAX_NODES]; /*!<Adjacency static array list */
...
Status graph_insertEdge(Graph *g, const long nId1, const long nId2){
  Status st=OK;
  int ind_aux;
  ind_aux=find_node_index(g, nId1);
  if(g==NULL || nId1 == -1 || nId2 == -1 || ind_aux==-1)
    return ERROR;
  if(graph_areConnected(g, nId1, nId2)==TRUE){
    return st;
  }
  st=list_pushOrder(g->plconnect[ind_aux], &nId2, 1);
  if(st==OK)
    g->num_edges++;

  st=node_setNConnect(g->nodes[ind_aux], node_getNConnect(g->nodes[ind_aux])+1);
  return st;
}
