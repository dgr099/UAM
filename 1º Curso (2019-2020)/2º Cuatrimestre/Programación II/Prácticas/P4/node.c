...
int node_print(FILE *pf, const void *n){
	const Node *nodo=NULL;
	nodo=n;
	if (n == NULL || pf==NULL){
		return -1;
	}
	return (fprintf(pf, "[%s, %ld, %ld, %d, %d]", nodo->name, nodo->id, nodo->predecessorid, nodo->label, nodo->nConnect));
}

/*-----------------------------------------------------------------------------
Nombre: node_getPredecessorId
Descripcion: Obtiene el Id del nodo anterior
Argumentos de entrada:
1. n, el puntero al nodo
Retorno:
- predecessorid, el id del nodo anterior
- -1, en caso de error
-----------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------
Nombre: node_setPredecessorId
Descripcion: Establece el id del nodo anterior
Argumentos de entrada:
1. n, el puntero al nodo
2. predid, el id a establecer
Retorno:
- OK, si se ha terminado sin problemas
- ERROR en caso contrario
-----------------------------------------------------------------------------*/
