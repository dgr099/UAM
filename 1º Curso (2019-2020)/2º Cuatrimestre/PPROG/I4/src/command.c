...
struct _Command{
T_Command principalcmd; /*!< principal cmd */
char argument[CMD_LENGHT]; /*!<the argument of the cmd */
char Status[WORD_SIZE]; /*!< the result of the callback */
};
...
/*! \var cmd_to_str
    ...
    Details.
*/
char *cmd_to_str[N_CMD][N_CMDT] = {{"", "no command"}, {"", "unknown"}, {"e", "exit"}, {"t", "take"}, {"d", "drop"}, {"rl", "roll"}, {"m", "move"}, {"i", "inspect"}, {"tn", "turnon"}, {"tf", "turnoff"}, {"o", "open"}, {"s", "save"}, {"lo", "load"}, {"tl", "talk"}, {"a", "accuse"}, {"n", "next"}, {"b", "back"}, {"r", "right"}, {"l", "left"}};
...
Command *command_get_user_input(Command *cmd)
{
 ...
 fgets(line, WORD_SIZE, stdin);
  ...
  if(toks!=NULL)
  strcpy(input, toks);
  if(strncmp("t", line, 1) == 0 || strncmp("d", line, 1)==0 || strncmp("m", line, 1)==0 || strncmp("i", line, 1)==0 || strncmp("o", line, 1)==0 || strncmp("lo", line, 2)==0 || strncmp("s", line, 1)==0 || strncmp("t", line, 1)==0 ||  strncmp("a", line, 1)==0){
      toks1 = strtok(NULL, "\n");
      if(toks1!=NULL)
