...
struct _Set
{
  Id tabla[MAX_ID + 1]; /*!< array of identifiers */
...
STATUS set_dell(Set *set, Id id){
  ...
  set->tabla[i]=NO_ID;
  for( ; i!=set_get_num(set); i++){
    set->tabla[i]=set->tabla[i+1];
  }
  /*set->tabla[set->num-1]=NO_ID;*/
  set->num--;
  return OK;
}
...
Id  set_get_id_at_position(Set* set, int position){
  if(!set || position>=set_get_num(set) || position<0){
    return -1;
  }
...
STATUS set_update(Set *original, Set *final){
  int i, j;
  if(!original || !final){
    return ERROR;
  }
  for(i=0, j=set_get_num(original); i!=j; i++){
    set_dell(original, set_get_id_at_position(original, 0));
  }
  for(i=0, j=set_get_num(final); i!=j; i++){
    if(set_add(original, set_get_id_at_position(final, i))==ERROR){
      return ERROR;
    }
  }
  return OK;
