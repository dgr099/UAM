...
bucle : bucle_exp sentencias '}' {
        ...
        //El aux es para guardar la etiqueta al no poder emplear $$
        
        /*
        //metodo 1, juntando todo 
        | aux TOK_DO '{'{ //cuando recibo el tok do empezamos el inicio del do while
            P_RULE(110,"<bucle>::= do { <sentencias> } while (<exp>);");
            //escribo inicio del bucle do while
            do_while_inicio(alfa_utils_T.fasm, $1.etiqueta);
        } sentencias '}' TOK_WHILE '(' exp ')' ';'{
            ...
            CHECK_ERROR($9.tipo == BOOLEAN, "se esperaba una expresión de tipo boolean");
            ...
            while_exp_pila_do_while(alfa_utils_T.fasm, $9.es_direccion, $1.etiqueta);
        }
        */

        //método 2, separandolo en dos partes
        | TOK_DO '{' i_do_while sentencias '}' TOK_WHILE '(' exp ')' ';' { //cuando recibo el tok do empezamos el inicio del do while
            P_RULE(110,"<bucle>::= do { <sentencias> } while (<exp>);");
            //al finalizar las sentencias y recibir el while, la exp se guarda en la pila
            //compruebas que la condición sea booleana la expresión, en caso contrario muestras el error
            CHECK_ERROR($8.tipo == BOOLEAN, "se esperaba una expresión de tipo boolean");
            //pones el final del do_while; similar al while pero comprobará que se cumpla la condición 
            //para saltar al inicio en lugar de comprobar que no se cumpla para saltar al final
            while_exp_pila_do_while(alfa_utils_T.fasm, $8.es_direccion, $3.etiqueta);
        }
