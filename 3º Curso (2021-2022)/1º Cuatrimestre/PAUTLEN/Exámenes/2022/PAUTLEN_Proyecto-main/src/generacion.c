...
void do_while_inicio(FILE *fpasm, int etiqueta)
{
        ...
        fprintf(fpasm, "; do_while_inicio\n");
        fprintf(fpasm, "inicio_do_while%d:\n", etiqueta);
        /* Ahora vendria una condicion (mayor, menor o igual, etc.) */
}

void while_exp_pila_do_while(FILE *fpasm, int exp_es_variable, int etiqueta)
{
    if (!fpasm) {
            printf("Error fallo en compilador, fichero fpasm nulo");
            return;
    ...
    fprintf(fpasm,  "cmp eax, 0\n");
    //mientras se cumpla la condición vuelves a saltar
    fprintf(fpasm, "jne near inicio_do_while%d\n", etiqueta);
    /* Aqui vendria el final del while */
}
