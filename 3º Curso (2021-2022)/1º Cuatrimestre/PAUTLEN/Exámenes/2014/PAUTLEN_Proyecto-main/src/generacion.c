...
void compare_start(FILE *fpasm, int exp_es_variable, int etiqueta)
{
        if (!fpasm) {
                printf("Error fallo en compilador, fichero fpasm nulo");
                return;
        }
        fprintf(fpasm, "; compare inicio\n");
        fprintf(fpasm, "pop eax\n");
        if(exp_es_variable)
            fprintf(fpasm, "mov dword eax, [eax]\n");
        fprintf(fpasm, "cmp eax, 0\n");
        fprintf(fpasm, "je near equal%d\n", etiqueta);
        fprintf(fpasm, "jg near greater%d\n", etiqueta);
}

void equal_inicio(FILE *fpasm, int etiqueta)
{
        if (!fpasm) {
                printf("Error fallo en compilador, fichero fpasm nulo");
                return;
        }
        fprintf(fpasm, "; equal_inicio\n");
        fprintf(fpasm, "jmp near fin_compare%d\n", etiqueta);
        fprintf(fpasm, "equal%d:\n", etiqueta);
}

void greater_inicio(FILE *fpasm, int etiqueta)
{
        if (!fpasm) {
                printf("Error fallo en compilador, fichero fpasm nulo");
                return;
        }
        fprintf(fpasm, "; greater_inicio\n");
        fprintf(fpasm, "jmp near fin_compare%d\n", etiqueta);
        fprintf(fpasm, "greater%d:\n", etiqueta);
}

void compare_fin(FILE *fpasm, int etiqueta)
{
        if (!fpasm) {
                printf("Error fallo en compilador, fichero fpasm nulo");
                return;
        }
        fprintf(fpasm, "; compare_fin\n");
        fprintf(fpasm, "fin_compare%d:\n", etiqueta);
}
