...
%type <atributos> condicional aux
...
%type <atributos> if_exp if_sentencias compare
...
compare : aux TOK_COMPARE exp TOK_WITH exp '{' TOK_LESS {
    CHECK_ERROR($3.tipo == $5.tipo && $3.tipo==INT, "se esperaba una expresión de tipo entero.");
     //haces la resta para tener en pila el valor, si es +. entonces es mayor, si es - menor...
    restar(alfa_utils_T.fasm, $3.es_direccion, $5.es_direccion);
    compare_start(alfa_utils_T.fasm, 0, $1.etiqueta);
    } sentencias TOK_EQUAL{
        equal_inicio(alfa_utils_T.fasm, $1.etiqueta);
    } sentencias TOK_GREATER{
        greater_inicio(alfa_utils_T.fasm, $1.etiqueta);
    } sentencias '}' {
        compare_fin(alfa_utils_T.fasm, $1.etiqueta);
    };
aux : {
    $$.etiqueta=etiqueta++;
}     
