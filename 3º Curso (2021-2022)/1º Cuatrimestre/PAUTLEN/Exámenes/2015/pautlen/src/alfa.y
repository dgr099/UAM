...
%type <atributos> init_vector valores_vector
...
init_vector :TOK_INIT identificador_use {
                    //inicializamos valores auxiliares para el tema del vector
                    char err_msg[TAM_ERRMSG] = "";
                    sym_info* sym = NULL;
                    sprintf(err_msg, "Identificador [%s] inexistente", $2.lexema);
                    sym = sym_t_get_symb($2.lexema);
                    CHECK_ERROR(sym != NULL, err_msg);
                    sprintf(err_msg, "El identificador no es un vector", $2.lexema);
                ...
                {P_RULE(38,"<init_vector> ::= <identificador_use> '{' <valores_vector> '}' ';'>");
                    if(vector_para_init->tipo==INT){
                        while(pos_vector_actual<vector_para_init->size){
                            ...
                            sprintf(valor, "0");
                            escribir_operando(alfa_utils_T.fasm, valor, 0);
                            escribir_elemento_vector(alfa_utils_T.fasm, $2.lexema, vector_para_init->size, vector_para_init->elem);
                            pos_vector_actual++;
                        }
                    }
                };
...
valores_vector : exp{
                        ...
                        CHECK_ERROR($1.tipo==vector_para_init->tipo, "Los tipos no concuerdan xdxd");
                        CHECK_ERROR(vector_para_init->size>pos_vector_actual, "Tamaño excedido");
                        escribir_elemento_vector(alfa_utils_T.fasm, $1.lexema, vector_para_init->size, vector_para_init->elem);
                        pos_vector_actual++;
                    }
                | exp ';'{
                        P_RULE(38,"<valores_vector> ::= <exp> ; <valores_vector>");
                        CHECK_ERROR($1.tipo==vector_para_init->tipo, "Los tipos no concuerdan");
                        CHECK_ERROR(vector_para_init->size>pos_vector_actual, "Tamaño excedido");
                        escribir_elemento_vector(alfa_utils_T.fasm, $1.lexema, vector_para_init->size, vector_para_init->elem);
